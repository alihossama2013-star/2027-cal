...
(function () {
    ...
    energy: {
      ...
      work_done: {
        ...
        calculate: (vals) => {
          ...
          let cosTheta = Math.cos((thetaDeg * Math.PI) / 180);
          if (normTheta === 90 || normTheta === 270) cosTheta = 0;
          else if (normTheta === 0) cosTheta = 1;
          else if (normTheta === 180) cosTheta = -1;
          ...
          return {
            ...
            formula: `Formula: W = F × d × cos(θ)\nStep 1: cos(${thetaDeg}°) = ${formatNumber(cosTheta, 4)}\nStep 2: Effective Force = ${f} × ${formatNumber(cosTheta, 4)} = ${formatNumber(effectiveF)} N\nStep 3: W = ${formatNumber(effectiveF)} × ${d} = ${formatNumber(w)} Joules`
          };
        },
        svg: `
  ...
  let openCasioFracDialog = null;
  let closeCasioFracDialog = null;
  ...
  function initScientificCalculator() {
    ...
    // Natural Fraction Entry Dialog Controls
    const fracDialog = document.getElementById('casio-frac-dialog');
    const fracDialogClose = document.getElementById('btn-frac-dialog-close');
    const btnFracModeSimple = document.getElementById('btn-frac-mode-simple');
    const btnFracModeMixed = document.getElementById('btn-frac-mode-mixed');
    const btnFracInsert = document.getElementById('btn-frac-insert');
    const btnFracClear = document.getElementById('btn-frac-clear');
    const btnFracCancel = document.getElementById('btn-frac-cancel');
    const fracInputWhole = document.getElementById('frac-input-whole');
    const fracInputNum = document.getElementById('frac-input-num');
    const fracInputDen = document.getElementById('frac-input-den');
    const fracWholeGroup = document.getElementById('frac-whole-group');
    const fracDialogTitle = document.getElementById('frac-dialog-title');
    const fracPreviewMath = document.getElementById('frac-preview-math');
    const fracPreviewError = document.getElementById('frac-preview-error');

    let dialogPendingTrailingNum = null;

    function setFracDialogMode(isMixed) {
      if (isMixed) {
        if (fracWholeGroup) fracWholeGroup.classList.remove('hidden');
        if (btnFracModeMixed) {
          btnFracModeMixed.classList.add('active');
          btnFracModeMixed.setAttribute('aria-selected', 'true');
        }
        if (btnFracModeSimple) {
          btnFracModeSimple.classList.remove('active');
          btnFracModeSimple.setAttribute('aria-selected', 'false');
        }
        if (fracDialogTitle) fracDialogTitle.textContent = 'Mixed Fraction (a b/c)';
        if (btnFracInsert) {
          btnFracInsert.innerHTML = '<span>Insert Mixed</span> <kbd>↵</kbd>';
        }
      } else {
        if (fracWholeGroup) fracWholeGroup.classList.add('hidden');
        if (btnFracModeSimple) {
          btnFracModeSimple.classList.add('active');
          btnFracModeSimple.setAttribute('aria-selected', 'true');
        }
        if (btnFracModeMixed) {
          btnFracModeMixed.classList.remove('active');
          btnFracModeMixed.setAttribute('aria-selected', 'false');
        }
        if (fracDialogTitle) fracDialogTitle.textContent = 'Fraction Entry (b/c)';
        if (btnFracInsert) {
          btnFracInsert.innerHTML = '<span>Insert Fraction</span> <kbd>↵</kbd>';
        }
      }
      updateFracDialogPreview();
    }

    function updateFracDialogPreview() {
      if (!fracPreviewMath) return;

      const isMixed = fracWholeGroup && !fracWholeGroup.classList.contains('hidden');
      const wholeRaw = fracInputWhole ? fracInputWhole.value.trim() : '';
      const numRaw = fracInputNum ? fracInputNum.value.trim() : '';
      const denRaw = fracInputDen ? fracInputDen.value.trim() : '';

      // Reset error state
      if (fracPreviewError) {
        fracPreviewError.textContent = '';
        fracPreviewError.classList.add('hidden');
      }
      if (btnFracInsert) btnFracInsert.disabled = false;

      // When everything is empty
      if (!numRaw && !denRaw && (!isMixed || !wholeRaw)) {
        fracPreviewMath.innerHTML = '<span class="preview-empty">Enter numbers above</span>';
        return;
      }
      ...
      if (denRaw === '0') {
        if (fracPreviewError) {
          fracPreviewError.textContent = '⚠ Denominator cannot be zero';
          fracPreviewError.classList.remove('hidden');
        }
        if (btnFracInsert) btnFracInsert.disabled = true;
        fracPreviewMath.innerHTML = '<span class="preview-empty" style="color: #f87171;">Undefined (division by 0)</span>';
        return;
      }

      const num = numRaw !== '' ? parseFloat(numRaw) : NaN;
      const den = denRaw !== '' ? parseFloat(denRaw) : NaN;
      const whole = (isMixed && wholeRaw !== '') ? parseFloat(wholeRaw) : 0;

      // Validate parsing
      if ((numRaw !== '' && isNaN(num)) || (denRaw !== '' && isNaN(den)) || (isMixed && wholeRaw !== '' && isNaN(whole))) {
        if (fracPreviewError) {
          fracPreviewError.textContent = '⚠ Please enter valid numeric values';
          fracPreviewError.classList.remove('hidden');
        }
        if (btnFracInsert) btnFracInsert.disabled = true;
        return;
      ...
      // Fully specified fraction
      if (!isNaN(num) && !isNaN(den) && den !== 0) {
        if (isMixed && wholeRaw !== '') {
          // Mathematically accurate mixed fraction calculation:
          // For positive whole: w + (num / den)
          // For negative whole: -( |w| + (num / den) )
          const isNeg = whole < 0;
          const absWhole = Math.abs(whole);
          const sign = isNeg ? -1 : 1;
          const absNum = Math.abs(num);
          const absDen = Math.abs(den);
          const decimalVal = sign * (absWhole + absNum / absDen);
          const improperNum = sign * (absWhole * absDen + absNum);

          const signPrefix = isNeg ? '−' : '';
          fracPreviewMath.innerHTML = `
            ${signPrefix}<span class="preview-whole">${absWhole}</span>
            <span class="preview-frac">${absNum}/${absDen}</span>
            <span class="preview-dec">= ${formatNumber(decimalVal, 6)}</span>
            <span class="preview-improper">(${improperNum}/${absDen})</span>
          `;
        } else {
          const decimalVal = num / den;
          fracPreviewMath.innerHTML = `
            <span class="preview-frac">${num}/${den}</span>
            <span class="preview-dec">= ${formatNumber(decimalVal, 6)}</span>
          `;
        }
      } else if (isMixed && wholeRaw !== '' && !isNaN(whole)) {
        // Only whole specified so far
        fracPreviewMath.innerHTML = `<span class="preview-whole">Whole: ${whole}</span> <span class="preview-empty">(enter fraction)</span>`;
      } else {
        fracPreviewMath.innerHTML = '<span class="preview-empty">Completing fraction...</span>';
      }
    }

    openCasioFracDialog = function (isMixed = false) {
      if (!fracDialog) return;
      fracDialog.classList.remove('hidden');

      // Check if expression ends with a typed number to prefill
      const expr = sciState.expression || '';
      const trailingNumMatch = expr.match(/(-?\d+(?:\.\d+)?)$/);
      if (trailingNumMatch && !sciState.hasEvaluated) {
        dialogPendingTrailingNum = trailingNumMatch[1];
      } else {
        dialogPendingTrailingNum = null;
      }

      setFracDialogMode(isMixed);

      if (isMixed) {
        if (dialogPendingTrailingNum) {
          if (fracInputWhole) fracInputWhole.value = dialogPendingTrailingNum;
          if (fracInputNum) {
            fracInputNum.value = '';
            fracInputNum.focus();
          }
          ...
          if (fracInputWhole) {
            fracInputWhole.value = '';
            fracInputWhole.focus();
          }
          if (fracInputNum) fracInputNum.value = '';
        }
        if (fracInputDen) fracInputDen.value = '';
      } else {
        if (dialogPendingTrailingNum) {
          if (fracInputNum) fracInputNum.value = dialogPendingTrailingNum;
          if (fracInputDen) {
            fracInputDen.value = '';
            fracInputDen.focus();
          }
        ...
        if (fracInputWhole) fracInputWhole.value = '';
      }

      updateFracDialogPreview();
    };

    closeCasioFracDialog = function () {
      if (!fracDialog) return;
      fracDialog.classList.add('hidden');
      dialogPendingTrailingNum = null;
      if (fracInputWhole) fracInputWhole.value = '';
      if (fracInputNum) fracInputNum.value = '';
      if (fracInputDen) fracInputDen.value = '';
      if (fracPreviewError) fracPreviewError.classList.add('hidden');
    };

    function clearFracDialogFields() {
      if (fracInputWhole) fracInputWhole.value = '';
      if (fracInputNum) fracInputNum.value = '';
      if (fracInputDen) fracInputDen.value = '';
      dialogPendingTrailingNum = null;
      const isMixed = fracWholeGroup && !fracWholeGroup.classList.contains('hidden');
      if (isMixed && fracInputWhole) {
        fracInputWhole.focus();
      } else if (fracInputNum) {
        fracInputNum.focus();
      }
      updateFracDialogPreview();
    }

    function insertFractionFromDialog() {
      const isMixed = fracWholeGroup && !fracWholeGroup.classList.contains('hidden');
      const wholeVal = fracInputWhole ? fracInputWhole.value.trim() : '';
      const numVal = fracInputNum ? fracInputNum.value.trim() : '';
      const denVal = fracInputDen ? fracInputDen.value.trim() : '';

      if (!numVal || !denVal) {
        showToast('Please enter both numerator and denominator');
        if (!numVal && fracInputNum) fracInputNum.focus();
        else if (fracInputDen) fracInputDen.focus();
        return;
      }

      const num = parseFloat(numVal);
      const den = parseFloat(denVal);

      if (isNaN(num) || isNaN(den)) {
        showToast('Numerator and denominator must be valid numbers');
        return;
      }

      if (den === 0) {
        showToast('Denominator cannot be zero');
        if (fracInputDen) fracInputDen.focus();
        return;
      }
      ...
      if (isMixed && wholeVal !== '') {
        ...
        if (isNaN(whole)) {
          showToast('Whole number must be a valid number');
          if (fracInputWhole) fracInputWhole.focus();
          return;
        }

        // Mathematically correct handling of negative whole numbers:
        // -2 1/2 evaluates to -(2 + 1/2) = -2.5
        if (whole < 0) {
          const absWhole = Math.abs(whole);
          const absNum = Math.abs(num);
          const absDen = Math.abs(den);
          insertText = `(-(${absWhole}+${absNum}/${absDen}))`;
        } else if (whole === 0) {
          insertText = `(${num}/${den})`;
        } else {
          insertText = `(${whole}+${Math.abs(num)}/${Math.abs(den)})`;
        }
      } else {
        insertText = `(${num}/${den})`;
      }
      ...
      if (sciState.hasEvaluated) {
        sciState.expression = insertText;
        sciState.hasEvaluated = false;
      } else if (dialogPendingTrailingNum !== null && sciState.expression.endsWith(dialogPendingTrailingNum)) {
        // Cleanly replace the prefilled trailing number
        sciState.expression = sciState.expression.slice(0, -dialogPendingTrailingNum.length) + insertText;
      } else {
        const expr = sciState.expression;
        if (expr && /[0-9.)Ansπe]$/.test(expr.trim())) {
          sciState.expression += `×${insertText}`;
        } else {
          sciState.expression += insertText;
        }
      ...
      dialogPendingTrailingNum = null;
      closeCasioFracDialog();
      updateCasioDisplay();
      showToast('Fraction inserted');
    }

    if (fracDialogClose) fracDialogClose.addEventListener('click', closeCasioFracDialog);
    if (btnFracCancel) btnFracCancel.addEventListener('click', closeCasioFracDialog);
    if (btnFracClear) btnFracClear.addEventListener('click', clearFracDialogFields);
    if (btnFracInsert) btnFracInsert.addEventListener('click', insertFractionFromDialog);

    if (btnFracModeSimple) {
      btnFracModeSimple.addEventListener('click', () => setFracDialogMode(false));
    }
    if (btnFracModeMixed) {
      btnFracModeMixed.addEventListener('click', () => setFracDialogMode(true));
    }

    // Live preview input events
    [fracInputWhole, fracInputNum, fracInputDen].forEach((inp) => {
      if (inp) {
        inp.addEventListener('input', updateFracDialogPreview);
      }
    });

    // Keyboard navigation and shortcuts inside dialog
    if (fracInputWhole) {
      fracInputWhole.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowRight' || e.key === 'Tab' && !e.shiftKey) {
          if (e.key === 'ArrowRight') {
            e.preventDefault();
            if (fracInputNum) fracInputNum.focus();
          }
    ...
    if (fracInputNum) {
      fracInputNum.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          if (fracInputDen) fracInputDen.focus();
    ...
    if (fracInputDen) {
      fracInputDen.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowUp') {
          e.preventDefault();
          if (fracInputNum) fracInputNum.focus();
        } else if (e.key === 'ArrowLeft') {
    ...
    if (copySciBtn) {
      copySciBtn.addEventListener('click', () => {
        const val = sciState.result || (resultDisplay ? resultDisplay.textContent : '0');
    ...
    if (resultDisplay) {
      resultDisplay.addEventListener('click', () => {
        const val = sciState.result || resultDisplay.textContent;
        if (val && val !== '0' && val !== '--') {
          copyToClipboard(val, copySciBtn || resultDisplay, 'Calculator result copied!');
        }
      });
    }

    // Visual button press helper for physical keyboard typing
    function triggerKeypadFeedback(selector) {
      if (!selector) return;
      try {
    ...
    // Physical keyboard event listeners for scientific calculator
    window.addEventListener('keydown', (e) => {
      // Allow browser shortcuts like Ctrl+C, Ctrl+V, Ctrl+R, etc.
      if (e.ctrlKey || e.metaKey || e.altKey) return;

      // Only process when scientific calculator panel is active
      const sciPanel = document.getElementById('sci-calc-panel');
      if (!sciPanel || !sciPanel.classList.contains('active')) return;

      // Do not hijack typing if focused inside an input, textarea, or select element
      ...
      if (/[0-9]/.test(key)) {
        ...
        triggerKeypadFeedback(`.casio-btn[data-val="${key}"]`);
        return;
      }

      // Decimal point (period or numpad comma)
      if (key === '.' || key === ',') {
        e.preventDefault();
        handleCasioAction('insert', key === ',' ? ',' : '.');
        triggerKeypadFeedback(key === ',' ? null : '.casio-btn[data-val="."]');
        return;
      }

      // Basic mathematical operators: +, -, *, /, x, X, ^, %
      if (key === '+') {
        e.preventDefault();
        handleCasioAction('insert', '+');
        triggerKeypadFeedback('.casio-btn[data-val="+"]');
        return;
      }
      ...
      if (key === '/') {
        e.preventDefault(); // Prevents browser quick search in Firefox
        handleCasioAction('insert', '÷');
        triggerKeypadFeedback('.casio-btn[data-val="÷"]');
        return;
      }
      ...
      // Mathematical constants and Ans
      if (key === 'p' || key === 'P') {
        e.preventDefault();
        handleCasioAction('insert', 'π');
        triggerKeypadFeedback('.casio-btn[data-val="π"]');
        return;
      }

      if (key === 'e' || key === 'E') {
        e.preventDefault();
        handleCasioAction('insert', 'e');
        triggerKeypadFeedback('.casio-btn[data-val="e"]');
        return;
      }

      if (key === 'a' || key === 'A') {
        e.preventDefault();
        handleCasioAction('ans');
        triggerKeypadFeedback('.casio-btn[data-action="ans"]');
        return;
      }

      // Fast function shortcuts
      if (key === 's' || key === 'S') {
        e.preventDefault();
        handleCasioAction('insert-func', 'sin');
        triggerKeypadFeedback('.casio-btn[data-val="sin"]');
        return;
      }

      if (key === 'c' || key === 'C') {
        e.preventDefault();
        handleCasioAction('insert-func', 'cos');
        triggerKeypadFeedback('.casio-btn[data-val="cos"]');
        return;
      }

      if (key === 't' || key === 'T') {
        e.preventDefault();
        handleCasioAction('insert-func', 'tan');
        triggerKeypadFeedback('.casio-btn[data-val="tan"]');
        return;
      }

      if (key === 'r' || key === 'R') {
        e.preventDefault();
        handleCasioAction('insert-func', 'sqrt');
        triggerKeypadFeedback('#btn-casio-root');
        return;
      }

      if (key === 'l' || key === 'L') {
        e.preventDefault();
        handleCasioAction('insert-func', 'ln');
        triggerKeypadFeedback('.casio-btn[data-val="ln"]');
        return;
      }

      // Fraction shortcut: 'f' or '\' for simple fraction (b/c), Shift+'F' or 'F' for mixed fraction (a b/c)
      if (key === 'f' || key === 'F' || key === '\\') {
        e.preventDefault();
        if (e.shiftKey || key === 'F') {
          sciState.shiftActive = true;
        }
        handleCasioAction('fraction');
        triggerKeypadFeedback('#btn-casio-frac');
        return;
      }

      // S-D toggle shortcut: 'd' or 'D'
      if (key === 'd' || key === 'D') {
        e.preventDefault();
        handleCasioAction('sd-toggle');
        triggerKeypadFeedback('#btn-casio-sd');
        return;
      }

      // Calculation, backspace, and clear controls
      if (key === 'Enter' || key === '=') {
        e.preventDefault();
        handleCasioAction('calculate');
        triggerKeypadFeedback('.casio-btn-eq');
        return;
      }
      ...
      if (key === 'Backspace') {
        ...
        triggerKeypadFeedback('.casio-btn-del');
        return;
      }

      if (key === 'Delete' || key === 'Escape') {
        e.preventDefault();
        if (typeof closeCasioFracDialog === 'function') {
          closeCasioFracDialog();
        }
        handleCasioAction('clear');
        triggerKeypadFeedback('.casio-btn-ac');
        return;
      }
    });
  ...
  function handleCasioAction(action, val) {
    switch (action) {
      case 'insert':
        if (sciState.hasEvaluated) {
          // If starting fresh with a number, decimal, or negative sign, clear expression
          if (/[0-9.]/.test(val) || val === '(-)') {
            sciState.expression = '';
          } else if (/^[+×÷\^]/.test(val) || (val === '-' && !sciState.expression)) {
            // Operator continues previous Ans!
        ...
        if (sciState.shiftActive) {
          sciState.shiftActive = false;
          updateShiftLabels();
        }
        break;

      case 'fraction': {
        const isShift = sciState.shiftActive;
        if (isShift) {
          sciState.shiftActive = false;
          updateShiftLabels();
        }

        // 1. If evaluated, pressing fraction button toggles S-D display
        if (sciState.hasEvaluated) {
        ...
        // 2. Open Natural Fraction entry helper!
        // If Shift was active, open in Mixed Fraction mode (a b/c); otherwise simple fraction (b/c).
        // Any trailing typed number is automatically pre-filled and smoothly replaced upon insertion.
        if (typeof openCasioFracDialog === 'function') {
          openCasioFracDialog(isShift);
        } else {
          sciState.expression += '/';
          updateCasioDisplay();
        }
      ...
      case 'sd-toggle':
        ...
        if (frac.den === 1) {
          ...
          showToast(`Standard: ${sciState.result}`);
          ...
          showToast(`Fraction: ${sciState.result}`);
          ...
          if (Math.abs(frac.num) > frac.den) {
            ...
            const rem = absNum % frac.den;
            const signedWhole = isNeg ? -whole : whole;
            sciState.result = `${signedWhole} ${rem}/${frac.den}`;
            sciState.sdState = 'mixed';
            showToast(`Mixed Fraction: ${sciState.result}`);
          } else {
            sciState.result = formatNumber(sciState.numericResult, 8);
            sciState.sdState = 'decimal';
            showToast(`Decimal: ${sciState.result}`);
          }
        } else {
      ...
      case 'insert-func':
        ...
        if (val === 'nCr' || val === 'nPr') {
          // Infix notation support: "5 nCr 2" or prefix "nCr("
          if (/[0-9.)]$/.test(sciState.expression.trim())) {
            sciState.expression += ` ${val} `;
          } else {
            sciState.expression += `${val}(`;
          }
      ...
      case 'toggle-angle':
        ...
        if (sciState.expression && sciState.expression.trim()) {
          const rt = calculateRealtimeCasioResult(sciState.expression, sciState.angleMode, sciState.ans);
          if (rt && typeof rt.numeric === 'number' && !isNaN(rt.numeric) && isFinite(rt.numeric)) {
            sciState.numericResult = rt.numeric;
            sciState.result = rt.text;
          }
        }
        showToast(`Angle mode: ${sciState.angleMode}`);
        break;
      ...
      case 'mem-add':
        ...
        const currentNum = sciState.numericResult;
        if (currentNum !== null && !isNaN(currentNum) && isFinite(currentNum)) {
      ...
      case 'mem-sub':
        ...
        const numSub = sciState.numericResult;
        if (numSub !== null && !isNaN(numSub) && isFinite(numSub)) {
  ...
  function updateShiftLabels() {
    ...
    if (sciState.shiftActive) {
      if (fracBtn) {
        fracBtn.setAttribute('title', 'Mixed Fraction (a b/c) [Hotkey: F]');
        fracBtn.innerHTML = `
          <span class="casio-frac-icon mixed" aria-hidden="true">
            <span class="frac-whole">a</span>
            <span class="frac-stacked">
              <span class="frac-box"></span>
      ...
      if (fracBtn) {
        fracBtn.setAttribute('title', 'Fraction (a/b) [Hotkey: F]');
        fracBtn.innerHTML = `
          <span class="casio-frac-icon" aria-hidden="true">
            <span class="frac-box frac-box-filled"></span>
            <span class="frac-line"></span>
            <span class="frac-box"></span>
          </span>
        `;
  ...
  // Real-time calculation helper that evaluates valid expressions and handles in-progress keystrokes
  function calculateRealtimeCasioResult(rawExpr, angleMode, ansVal) {
    if (!rawExpr || !rawExpr.trim()) {
      return { numeric: 0, text: '0', isComplete: true };
    }

    const expr = rawExpr.trim();

    // 1. Direct evaluation: handles full expressions, complete operations, and auto-balanced parens
    try {
      const parsed = parseAndCompute(expr, angleMode, ansVal);
      if (typeof parsed === 'number' && !isNaN(parsed) && isFinite(parsed)) {
        return { numeric: parsed, text: formatNumber(parsed, 8), isComplete: true };
      }
    } catch (e) {
      // Direct parse failed (likely trailing operator or unclosed function call)
    }

    // 2. Progressive fallback: strip trailing incomplete operators or dangling function names
    try {
      let cleaned = expr;
      for (let attempt = 0; attempt < 6; attempt++) {
        const nextCleaned = cleaned
          .replace(/[+\-×*÷/^,%!\s]+$/, '')
          .replace(/\b(sin|cos|tan|asin|acos|atan|sinh|cosh|tanh|sqrt|cbrt|ln|log|abs|exp|fact|nCr|nPr)\s*\(?$/i, '')
          .replace(/\(\s*$/, '')
          .trim();

        if (!nextCleaned || nextCleaned === cleaned) {
          break;
        }
        cleaned = nextCleaned;

        try {
          const parsed = parseAndCompute(cleaned, angleMode, ansVal);
          if (typeof parsed === 'number' && !isNaN(parsed) && isFinite(parsed)) {
            return { numeric: parsed, text: formatNumber(parsed, 8), isComplete: false };
          }
        } catch (err) {
          // Continue peeling
        }
  ...
  function updateCasioDisplay() {
    ...
    const modeBadge = document.getElementById('casio-mode-badge');
    const memBadge = document.getElementById('casio-mem-badge');
    const realtimeBadge = document.getElementById('casio-realtime-badge');

    if (exprEl) exprEl.textContent = sciState.expression || '';

    // If not already explicitly committed with '=', compute result in real time
    if (!sciState.hasEvaluated) {
      if (!sciState.expression || !sciState.expression.trim()) {
        sciState.result = '0';
        sciState.numericResult = 0;
    ...
    if (resultEl) {
      resultEl.textContent = sciState.result || '0';
      if (!sciState.hasEvaluated && sciState.expression.trim()) {
        resultEl.classList.add('is-realtime');
        resultEl.setAttribute('title', 'Real-time result (Click to copy • Press = to record to history)');
      } else {
        resultEl.classList.remove('is-realtime');
        resultEl.setAttribute('title', 'Final result (Click to copy)');
      }
    }

    if (realtimeBadge) {
      if (!sciState.hasEvaluated && sciState.expression.trim()) {
        realtimeBadge.classList.add('active');
        realtimeBadge.style.opacity = '1';
      } else {
        realtimeBadge.style.opacity = '0.75';
      }
    }
  ...
  function evaluateCasioExpression(recordHistory = false) {
    ...
    try {
      ...
      if (isNaN(parsed) || !isFinite(parsed)) {
        ...
        if (recordHistory) showToast('Math ERROR: Invalid mathematical domain or division by zero');
      } else {
        sciState.numericResult = parsed;
        sciState.sdState = 'decimal';
        sciState.result = formatNumber(parsed, 8);
        sciState.ans = parsed;
        ...
        if (recordHistory) {
          ...
          showToast(`Evaluated: ${sciState.result} (Saved to History & Ans)`);
        }
      }
    } catch (err) {
      sciState.result = 'Syntax ERROR';
      sciState.numericResult = null;
      if (recordHistory) showToast('Syntax ERROR: Incomplete or invalid expression');
    }

    updateCasioDisplay();
  }

  // Safe, robust scientific math evaluator with proper operator precedence and domain validation
  function parseAndCompute(rawExpr, angleMode, ansVal) {
    ...
    str = str.replace(/\bAns\b/gi, `(${ansVal !== null && !isNaN(ansVal) ? ansVal : 0})`);
    str = str.replace(/√/g, 'sqrt ');

    // Support mixed fractions e.g. "1 1/2", "-2 1/2", "3_1/4", "3 1 / 2"
    // Handles positive and negative whole numbers mathematically: -2 1/2 -> (- (2 + 1/2))
    str = str.replace(/(^|[+\-×*÷/^(,])\s*(-)?(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)/g, (match, prefix, sign, whole, num, den) => {
      const isNeg = sign === '-';
      if (isNeg) {
    ...
    str = str.replace(/(^|[+\-×*÷/^(,])\s*(-)?(\d+(?:\.\d+)?)[_\u2518\u231D](\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)/g, (match, prefix, sign, whole, num, den) => {
      const isNeg = sign === '-';
      if (isNeg) {
        return `${prefix}(-(${whole}+${num}/${den}))`;
      } else {
    ...
    // Auto-balance closing parentheses
    let openCount = 0;
    for (let idx = 0; idx < str.length; idx++) {
      if (str[idx] === '(') openCount++;
      else if (str[idx] === ')') openCount--;
    }
    if (openCount > 0) {
    ...
    while (i < n) {
      ...
      // Numbers (integers, decimals, exponents e.g. 1.2e-4)
      if (/[0-9]/.test(ch) || (ch === '.' && i + 1 < n && /[0-9]/.test(str[i + 1]))) {
        let numStr = '';
        while (i < n && /[0-9.]/.test(str[i])) {
          numStr += str[i];
          i++;
        }
        if (i < n && /[eE]/.test(str[i])) {
          let expStr = str[i];
          let j = i + 1;
          if (j < n && /[+\-]/.test(str[j])) {
            expStr += str[j];
      ...
      // Identifiers / Constants / Words / π
      if (/[a-zA-Z_π]/.test(ch)) {
        let id = '';
        while (i < n && /[a-zA-Z0-9_π]/.test(str[i])) {
          id += str[i];
          i++;
        }
        rawTokens.push({ type: 'ID', value: id });
        continue;
      }

      // Operators and punctuation
      if ('+-*/^!(),%'.includes(ch)) {
        rawTokens.push({ type: 'PUNCT', value: ch });
        i++;
        continue;
      }
    ...
    // Insert implicit multiplication tokens e.g. 2(3), (2)(3), 2pi, 5sin(30), 5!2
    const tokens = [];
    for (let k = 0; k < rawTokens.length; k++) {
      ...
      if (k + 1 < rawTokens.length) {
        ...
        const isCurrOperand = (curr.type === 'NUM') ||
          (curr.type === 'ID' && ['pi', 'PI', 'π', 'e', 'E', 'ans', 'Ans'].includes(curr.value)) ||
          (curr.type === 'PUNCT' && (curr.value === ')' || curr.value === '!'));

        const isNextStart = (next.type === 'NUM') ||
          (next.type === 'ID' && !['nCr', 'nPr', 'C', 'P'].includes(next.value)) ||
          (next.type === 'PUNCT' && next.value === '(');

        if (isCurrOperand && isNextStart) {
    ...
    function consume(expectedVal) {
      ...
      if (expectedVal !== undefined && tok.value !== expectedVal) {
        throw new Error(`Expected ${expectedVal} but got ${tok.value}`);
      }
      pos++;
      return tok;
    }
    ...
    function parseAddSub() {
      ...
      while (true) {
        const tok = peek();
        if (tok && tok.type === 'PUNCT' && (tok.value === '+' || tok.value === '-')) {
          consume();
      ...
      while (true) {
        ...
        if (tok.type === 'PUNCT' && (tok.value === '*' || tok.value === '/' || tok.value === '%')) {
          ...
          if (tok.value === '*') {
            ...
            if (right === 0) return NaN; // Division by zero -> Math ERROR
            left = left / right;
          } else if (tok.value === '%') {
            if (right === 0) return NaN;
            left = left % right;
          }
        } else if (tok.type === 'ID' && ['nCr', 'nPr', 'C', 'P'].includes(tok.value)) {
          consume();
          const right = parsePower();
          if (tok.value === 'nCr' || tok.value === 'C') {
            left = computeNCr(left, right);
          } else {
            left = computeNPr(left, right);
          }
        } else {
          break;
        }
    ...
    function parsePower() {
      ...
      if (tok && tok.type === 'PUNCT' && tok.value === '^') {
        ...
        if (!isFinite(res) || isNaN(res)) return NaN;
        return res;
      }
      return left;
    }
    ...
    function parsePrimary() {
      ...
      if (tok.type === 'ID') {
        ...
        if (name === 'ans') return (ansVal !== null && !isNaN(ansVal) ? ansVal : 0);

        // Functions with explicit arguments in parentheses
        if (peek() && peek().type === 'PUNCT' && peek().value === '(') {
          ...
          consume(')');
          return evaluateFunction(name, arg1, arg2, angleMode);
        }

        // Functions without explicit parentheses e.g. sqrt 16, sin 30
        if (['sqrt', 'cbrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'sinh', 'cosh', 'tanh', 'ln', 'log', 'abs', 'exp', 'fact'].includes(name)) {
          const arg = parsePrimary();
          return evaluateFunction(name, arg, null, angleMode);
        }

        throw new Error(`Unknown function or variable: ${tok.value}`);
      }

      throw new Error(`Unexpected token: ${tok.value}`);
    }

    const evaluated = parseExpr();
    if (pos < tokens.length) {
      throw new Error('Unparsed trailing tokens');
    }
    return evaluated;
  }

  function evaluateFunction(name, x, y, angleMode) {
    ...
    switch (name) {
      ...
      case 'tan': {
        if (angleMode === 'DEG') {
          ...
          if (norm === 90) return NaN; // Undefined -> Math ERROR
          if (norm === 0) return 0;
          if (norm === 45) return 1;
      ...
      case 'fact': return computeFactorial(x);

      case 'ncr':
      case 'c': {
        if (y === null) return NaN;
        return computeNCr(x, y);
      }

      case 'npr':
      case 'p': {
        if (y === null) return NaN;
        return computeNPr(x, y);
      }
  ...
  function computeFactorial(n) {
    ...
    if (n === 0 || n === 1) return 1;
    let res = 1;
    for (let idx = 2; idx <= n; idx++) res *= idx;
    return res;
  }

  function computeNCr(n, r) {
    if (typeof n !== 'number' || typeof r !== 'number') return NaN;
  ...
  function addHistoryEntry(entry) {
    // Prevent duplicate consecutive entries with identical type, inputs, and output
    if (calculationHistory.length > 0) {
      const first = calculationHistory[0];
      if (first.type === entry.type && first.inputs === entry.inputs && first.output === entry.output) {
